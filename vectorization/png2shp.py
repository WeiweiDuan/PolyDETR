    ...
    for (s, e) in G.edges():
        ...
        for val in vals:
            ...
            for p in range(0, len(ps)-1):
                cv2.line(img, (ps[p][1],ps[p][0]), (ps[p+1][1],ps[p+1][0]), (0, 255, 0), 3)
...
def remove_small_terminal(G):
    ...
    for s, e in edges:
        ...
        vals = flatten([[v] for v in G[s][e].values()])
        for ix, val in enumerate(vals):
            if s in terminal_points and val.get('weight', 0) < 10: #was 10
                G.remove_node(s)
            if e in terminal_points and val.get('weight', 0) < 10: #was 10
                G.remove_node(e)
    return


linestring = "LINESTRING {}"
def make_skeleton(root, debug, fix_borders):
    replicate = 5
    clip = 2
    rec = replicate + clip
    # open and skeletonize
    img_ = cv2.imread(root, cv2.IMREAD_GRAYSCALE)
    # img_ = (np.logical_and(img_>0, img_<150)*1*255).astype(np.uint8)
    kernel = np.ones((3,3),np.uint8)
    img = cv2.dilate(img_,kernel,iterations = 5)
...
def add_small_segments(G, terminal_points, terminal_lines):
    ...
    possible2 = np.argwhere((dists > 0) & (dists < 100))
...
def build_graph(root, COOR_SYS_PATH, EPSG, OUTPUT_PATH, debug=False, add_small=True, fix_borders=True):
    print('************ processing %s ************'%(root))
    img_copy, ske_entire = make_skeleton(root, debug, fix_borders)
#     ske_entire = cv2.imread(root, 0)
    step_r, step_c = 3000, 5000 
    for row in range(0, ske_entire.shape[0], step_r):
        for col in range(0, ske_entire.shape[1], step_c):
            if not os.path.isfile(OUTPUT_PATH):
                is_append = False
            else:
                is_append = True
            ske = copy.deepcopy(ske_entire)
            zeros = np.zeros_like(ske_entire)
            zeros[row:row+step_r, col:col+step_c] = 1
            ske = zeros * ske_entire
            ...
            shp_writer.createShapefile(OUTPUT_PATH, wkt, COOR_SYS_PATH, EPSG, append=is_append)
    return

if __name__ == "__main__":
    res = build_graph(RASTER_PATH, COOR_SYS_PATH, EPSG, OUTPUT_PATH, debug=False)
